...
public class ReceiptService {
    ...
    public List<Map<String, Object>> getAllReceipts() {
        ...
        for (Receipt receipt : receipts) {
            ...
            BigDecimal totalPriceOfReceipt = BigDecimal.ZERO;
            ...
            for (ItemReceipt rp : itemReceipts) {
                ...
                totalPriceOfReceipt = totalPriceOfReceipt.add(totalPriceOfItem);
            }

            receiptData.put("proizvodi", itemsData);
            receiptData.put("ukupna_cijena_racuna", totalPriceOfReceipt.setScale(2, BigDecimal.ROUND_HALF_UP));
    ...
    public Map<String, Object> getReceiptDetailsById(int id) {
        ...
        List<ItemReceipt> itemsReceipt = itemReceiptRepository.findByReceipt(receipt);
        List<Map<String, Object>> itemsData = new ArrayList<>();

        BigDecimal totalPriceOfReceipt = BigDecimal.ZERO;

        for (ItemReceipt rp : itemsReceipt) {
            Map<String, Object> itemData = new HashMap<>();
            ...
            BigDecimal totalPriceItem = pricePerUnit.multiply(amount).setScale(2, BigDecimal.ROUND_HALF_UP);

            itemData.put("imeProizvoda", itemName);
            itemData.put("kolicina", amount);
            itemData.put("cijena_po_jedinici", pricePerUnit);
            itemData.put("ukupna_cijena_proizvoda", totalPriceItem);

            itemsData.add(itemData);

            totalPriceOfReceipt = totalPriceOfReceipt.add(totalPriceItem);
        }
